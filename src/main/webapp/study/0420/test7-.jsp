  ...
  <script>
    ...
    function deleteBox(idx) {
    	...
    	item.parentNode.removeChild(item);
			...
			//idx 번째의 값이 지워지면 cnt배열의 값도 지운다.
    	cnt = cnt.filter((element) => {
    		  return element !== idx;
    	});
    }
    ...
    function fCheck() {
    	...
    	for(let i=0; i<products.length; i++) {
    		//console.log("length",products.length, "i",i);
    		//d = cnt.slice(i,1);
 				//console.log("d",d,"cnt",cnt);
    		d = cnt[i];
    		if(products[i].value == "") {
    			alert("상품명을 입력하세요");
    			console.log("dd",d,"cccnt",cnt);
    			document.getElementById("product"+d).focus();
    			return false;
    		}
    			...
    			alert("수량을 입력하세요");
    			document.getElementById("su"+d).focus();
    			return false;
    		}
    	}
    	myform.submit();

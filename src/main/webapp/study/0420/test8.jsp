...
<head>
  ...
  <!-- <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.4/jquery.min.js"></script> -->
	<script>
		...
		// 박스추가버튼을 클릭할 때마다 입력폼 동적 추가 : 각 TextField마다 고유한 id를 지정해야 한다.
		function inputBox() {
			idx++;
			...
			newText += '<td><input type="hidden" name="cnt" id="su'+idx+'" /></td>';
			newText += '<td><input type="button" value="삭제" onclick="deleteBox('+idx+')" class="btn btn-secondary" /></td>';
			newText += '</tr>';
			newText += '</table>';
			newText += '</div>';
			
			demo.innerHTML += newText;   //  누적이 되게 써줘야 한다!!! 주의하자!!!!
		}
		
		// 삭제버튼을 클릭할 때마다 해당 테이블 박스 1개만 삭제
		function deleteBox(idx) {
			let proTxt = "proTxt" + idx();
			let item = document.getElementByIdd(proTxt);
			item.parentNode.removerChild(item);
			
	/* 		for(let i=0; i<cnt.length; i++) {
				if(i == idx-1) cnt[i] = null;
			}
			for(let i=0; i<cnt.length; )
		} */
...
/* 			for(let i=0; i<products.length; i++) { */
			for(let i=0; i<cnt.length; i++) {
				if(products[i].value == "") {
					alert("상품명을 입력하세요");
//					document.getElementById("product"+i).focus();  // 지웠으니까 사실 이렇게 쓸 순 없다.
					document.getElementById("product"+cnt[i]).focus();  // 지웠으니까 사실 이렇게 쓸 순 없다.
					return false;
				}
				else if(prices[i].value == "") {
					alert("가격을 입력하세요");
					document.getElementById("price"+i).focus();  // 지웠으니까 사실 이렇게 쓸 순 없다.
					return false;
				}
				else if(sus[i].value == "") {
					alert("수량을 입력하세요");
					document.getElementById("su"+i).focus();  // 지웠으니까 사실 이렇게 쓸 순 없다.
					return false;
				}
			}
			myform.submit();		
	 				...
	 				<td colspan="4" class="text-center">
	 					<table class="table">
	 						<tr>
	 							...
	 							<td><input type="hidden" name="test" value="0" /></td>
	 							<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </td>
	 						</tr>
	 					</table>
	 					<div id="demo"></div>

...
<head>
	...
	<script>
		...
		function fileBoxAppend() {
			...
			fileBox += '<input type="file" name="fName'+cnt+'" id="file'+cnt+'" class="form-control border mb-2" style="float:left; width:80%" />';
			fileBox += '<input type="button" value="삭제" onclick="deleteBox('+cnt+')" class="btn btn-danger form-control mb-2 ml-3" style="width:15%"/>';
			fileBox += '</div>';
...
<div class="container">	
	...
	<form name="myform" method="post" action="${ctp}/FileUpLoad3Ok.st" enctype="multipart/form-data">
		파일명 : 
		<div>
			<input type="button" value="파일박스 추가" onclick="fileBoxAppend()" class="btn btn-info mb-4"/>

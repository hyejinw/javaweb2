...
<div class="container">
  ...
  <table class="table table-borderless mb-0 p-0">
    <tr>
      <td>
        ...
        <c:if test="${sAdmin == 'adminOk'}"><a href="${ctp}/AdminLogout.gu" class="btn btn-secondary btn-sm">관리자 로그아웃</a></c:if>
      </td>
	  ...
	  <table class="table table-borderless mb-0 mt-0">
	    <tr>
	      <td>
	      	<%-- 번호 : ${vo.idx} --%>
	      	번호 : ${curScrStartNo}
	      	<c:if test="${sAdmin == 'adminOk'}">  <!-- 삭제할 때 바로없애면 정없어!! 조심해야 한다. 매개 변수에 숫자와 문자가 갈때는 전부 문자로 보내는 게 좋다.-->
  ...
  <div class="text-center">
  	<ul class="pagination justify-content-center">
	    <c:if test="${pag > 1}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/GuestList.gu?pageSize=${pageSize}&pag=1">◁◁</a></li></c:if>
	    ...
	    <c:if test="${pag < totPage}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/GuestList.gu?pageSize=${pageSize}&pag=${totPage}">▷▷</a></li></c:if>
 		</ul>
  </div>
</div>
<p><br/></p>

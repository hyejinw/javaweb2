      ...
      response.addCookie(cookieMid);
		...
		// 로그인 했을 때 DB에 값이 없는 경우
			...
			if(vo.getFailCount() == 0) {
				System.out.println(vo.getFailCount());
				int failCount = 1; 
				vo.setFailCount(failCount);
				out.print("<script>");
				out.print("alert('로그인 1회 실패');");
				out.print("location.href='"+request.getContextPath()+"/study/0428_database/login.jsp';");
				out.print("</script>");
			}
			// 회원 인증 실패 시 처리
			else if(vo.getFailCount() < 5) {
				int failCount = vo.getFailCount();   // 로그인 실패 횟수 체크용
				vo.setFailCount(++failCount);
				...
				out.print("<script>");
				out.print("alert('로그인 인증 5회 실패. 비밀번호 찾기 페이지로 이동합니다.');");

...
public class PdsInputOkCommand implements PdsInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int maxSize = 1024 * 1024 * 20;	// 서버에 저장할 최대용량을 20MByte로 제한한다.(1회저장용량)
		...
		if(res == 1) {
			request.setAttribute("msg", "자료실에 업로드 되었습니다.");
			request.setAttribute("url", request.getContextPath()+"/PdsList.pds");
		}
		else {
			request.setAttribute("msg", "자료실에 파일이 업로드되지 않았습니다.");
